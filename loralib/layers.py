...
class PlainMultiheadAttentionLoRA(nn.Module):
    ...
    def train(self, mode: bool = True):
        super().train(mode)
        # Set LoRA training mode for all LoRA-enabled projections
        if hasattr(self, 'q_proj') and isinstance(self.q_proj, LinearLoRA):
            self.q_proj.lora_train(mode)
        if hasattr(self, 'k_proj') and isinstance(self.k_proj, LinearLoRA):
            self.k_proj.lora_train(mode)
        if hasattr(self, 'v_proj') and isinstance(self.v_proj, LinearLoRA):
            self.v_proj.lora_train(mode)
        if hasattr(self, 'proj') and isinstance(self.proj, LinearLoRA):
            self.proj.lora_train(mode)  
    ...
    def init_BA_with_teacher(self, teacher):
        assert set(self.enable_lora) == set(teacher.enable_lora)
        for item in self.enable_lora:
            if item == 'o':
                proj_module = getattr(self, f"proj")
                teacher_proj_module = getattr(teacher, f"proj")
            else:
                proj_module = getattr(self, f"{item}_proj")
                teacher_proj_module = getattr(teacher, f"{item}_proj")

            # U: (d_out, min(d_out, d_in)), Vh: (min(d_out, d_in), d_in), S: (min(d_out, d_in), )
            U, S, Vh = torch.linalg.svd(teacher_proj_module.weight - proj_module.weight, full_matrices=False)  # (min(d_out, d_in), min(d_out, d_in))
            S_sqrt = torch.diag(torch.sqrt(S[:self.r]))
            B = U[:, :self.r] @ S_sqrt
            A = S_sqrt@Vh[:self.r, :]

            proj_module.w_lora_A.data.copy_(A)  # (r, d_in)
            proj_module.w_lora_B.data.copy_(B)  # (d_out, r)
