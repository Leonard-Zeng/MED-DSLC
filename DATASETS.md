...
### CUB200
- **`--dataset` key:** `cub200`
- Create a folder named `CUB200/` under `$DATA`.
- Place images under `CUB200/images/` using the usual CUB-200-2011 class folder layout (e.g. `001.Black_footed_Albatross/...`).
- Place `split_zhou_CUB200.json` inside `CUB200/` (Zhou-format split: lists of `[relative_path, label, classname]` under `train` / `val` / `test`; paths are relative to `images/`).

The directory structure should look like
```
CUB200/
|–– images/
|–– split_zhou_CUB200.json
```

### RESISC45
- **`--dataset` key:** `resisc45`
- Create a folder named `RESISC45/` under `$DATA`.
- Place images under `RESISC45/images/` with one subfolder per class (e.g. `river/river_066.jpg`).
- Place `split_zhou_RESISC45.json` inside `RESISC45/` (paths in the JSON relative to `images/`).

```
RESISC45/
|–– images/
|–– split_zhou_RESISC45.json
```

### AIBD Cars
- **`--dataset` key:** `aibd_cars`
- Create a folder named `AIBD_Cars/` under `$DATA`.
- Place images under `AIBD_Cars/images/` (paths in the split JSON relative to `images/`).
- Place `split_zhou_AIBDCars.json` inside `AIBD_Cars/`.

```
AIBD_Cars/
|–– images/
|–– split_zhou_AIBDCars.json
```

For **meta/MED training** (`training/train.py --include_new_ds`), `cub200` and `resisc45` are appended to the training domain list. AIBD Cars is not added as a training domain because it shares the Stanford Cars semantic class set.

For **benchmark evaluation** (`evaluation/eval_meta_benchmark.py --include_new_ds`), `cub200` and `resisc45` are appended, and `aibd_cars` replaces `stanford_cars` as the car-domain inference dataset.
