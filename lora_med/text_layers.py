...
class TextLinearMED(nn.Linear, TextMEDLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # set sequence_length from backbone constants (needed for 2D reshape)
        sequence_length = openclip_backbones[backbone]["text"]["sequence_length"]
    ...
    def forward(self, x: torch.Tensor, alpha, **kwargs):
        ...
        if alpha.shape[1] == self.total_rank:
            # rank-gating: alpha is (B, T) where T = sum(r_i)
            return self._forward_rank_gated(x, alpha, original_output)
        else:
            # expert-gating: alpha is (B, N) where N = num_experts
    ...
    def _forward_rank_gated(self, x: torch.Tensor, alpha: torch.Tensor, original_output: torch.Tensor):
        # Handle input shape: (L, B, d), (B, L, d), or (L*B, d)
...
class TextPlainMultiheadAttentionMED(nn.Module):
    def __init__(
            self,
            num_experts: int,
            r: list[int],
        ...
        # TextMEDLayer.__init__(self, num_experts=num_experts, r=r, lora_alpha=lora_alpha, dropout_rate=dropout_rate)
        # Per-layer gating uses true sequence length from backbone
        sequence_length = openclip_backbones[backbone]["text"]["sequence_length"]
        hidden_dim = openclip_backbones[backbone]["text"]["hidden_size"]
        self.sequence_length = sequence_length
        
        # create gating network based on gate_type
        if gate_type == "mole":
            ...
            raise NotImplementedError(f"Gate type '{gate_type}' is not implemented")
        
        # Init qkv as a new lora linear layer
        self.enable_lora = enable_lora
        for item in self.enable_lora:
            if item == 'q':
                self.q_proj = TextLinearMED(self.q_proj,
                ...
                self.k_proj = TextLinearMED(self.k_proj,
                                             num_experts=num_experts,
                                             r=r,
                                             lora_alpha=lora_alpha,
                                             fan_in_fan_out=False,
                ...
                self.v_proj = TextLinearMED(self.v_proj,
                                             num_experts=num_experts,
                                             r=r,
                                             lora_alpha=lora_alpha,
                                             fan_in_fan_out=False,
                ...
                self.proj = TextLinearMED(self.proj,
                                           num_experts=num_experts,
                                           r=r,
                                           lora_alpha=lora_alpha,
                                           fan_in_fan_out=False,
    ...
    def forward_module(
        ...
        """
        # Check for alpha_override (used by external joint-gating experiments)
        if hasattr(self, 'alpha_override') and self.alpha_override is not None:
            alpha = self.alpha_override  # (B_img*B_txt, E) - override from joint gating
        elif self.gate_type == "fixed":
            alpha = self.fixed_alpha.unsqueeze(0).expand(bsz, -1)
        else:
            alpha = self.gate_net(query, input_ids=self.input_ids_override)[0]
        # print(f"texts: {[ [ f'{v:.4f}' for v in vector] for vector in alpha]}")
        # print(f"texts: {alpha.argmax(-1)}")

        if type(self.q_proj) is TextLinearMED:
            q = self.q_proj(query, alpha)
        else:
            q = self.q_proj(query)
        if type(self.k_proj) is TextLinearMED:
            k = self.k_proj(key, alpha)
        else:
            k = self.k_proj(key)
        if type(self.v_proj) is TextLinearMED:
        ...
        if type(self.proj) is TextLinearMED:
